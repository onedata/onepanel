...
post_should_start_auto_storage_import_scan(Config) ->
    ?eachHost(Config, fun(Host) ->
        ?assertMatch(
            ...
            onepanel_test_rest:auth_request(
                Host, <<"/provider/spaces/someId/storage-import/auto/start">>, post,
                ?OZ_OR_ROOT_AUTHS(Host, [?CLUSTER_UPDATE])
            )
        ),
        ?assertReceivedMatch({service, oneprovider, start_auto_storage_import_scan, #{
            space_id := <<"someId">>
        }}, ?TIMEOUT)
    end).
...
post_should_stop_auto_storage_import_scan(Config) ->
    ?eachHost(Config, fun(Host) ->
        ?assertMatch(
            {ok, ?HTTP_204_NO_CONTENT, _, _},
            onepanel_test_rest:auth_request(
                Host, <<"/provider/spaces/someId/storage-import/auto/stop">>, post,
                ?OZ_OR_ROOT_AUTHS(Host, [?CLUSTER_UPDATE])
            )
        ),
        ?assertReceivedMatch({service, oneprovider, stop_auto_storage_import_scan, #{
            space_id := <<"someId">>
        }}, ?TIMEOUT)
    end).
