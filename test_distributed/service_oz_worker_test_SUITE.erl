...
%%% @end
%%%--------------------------------------------------------------------
-module(service_oz_worker_test_SUITE).
-author("Krzysztof Trzepla").
...
deploy_should_create_cluster(Config) ->
    ...
    ?assertEqual(ok, rpc:call(Node, service, apply,
        [?SERVICE, deploy, #{hosts => Hosts, main_cm_host => hd(Hosts),
...
stop_should_deactivate_service(Config) ->
    ...
    ?assertMatch({error, {service_oz_worker, status, {[], [_ | _]}}},
