...
%%% @end
%%%--------------------------------------------------------------------
-module(rest_couchbase_test_SUITE).
-author("Krzysztof Trzepla").
...
-define(SERVICE_MODULE, service_couchbase).
...
get_should_return_service_status(Config) ->
    ?assertMatch({ok, 200, _, _},
        do_request(Config, "/cluster/databases", get)).
...
patch_should_start_service_on_host(Config) ->
    ...
    lists:foreach(fun(Host) ->
        ?assertMatch({ok, 200, _, _}, do_request(Config,
            "/cluster/databases/" ++ Host ++ "?started=true", patch))
    end, Hosts).


patch_should_stop_service(Config) ->
    ?assertMatch({ok, 200, _, _}, do_request(Config,
        "/cluster/databases?started=false", patch)).
...
patch_should_stop_service_on_host(Config) ->
    ...
    lists:foreach(fun(Host) ->
        ?assertMatch({ok, 200, _, _}, do_request(Config,
            "/cluster/databases/" ++ Host ++ "?started=false", patch))
    end, Hosts).
