...
check_storage_paths(Hosts, [StoragePath | StoragePaths]) ->
    case installer_storage:check_storage_path_on_hosts(Hosts, StoragePath) of
        ...
        {error, {hosts, ErrorHosts}} ->
            onepanel_gui_utils:message(<<"top_menu">>, error, <<"Storage: ", (list_to_binary(StoragePath))/binary,
        ...
        receive
            ...
            next ->
                NextState = case StoragePaths of
                                [] ->
                                    onepanel_gui_utils:message(<<"top_menu">>, error, <<"Please add at least one storage.">>),
                                    State;
                                _ ->
            ...
            {add_storage_path, StorageId, StoragePath} ->
                NextState = case lists:member(StoragePath, StoragePaths) of
                                true ->
                                    onepanel_gui_utils:message(<<"top_menu">>, error, <<"Storage path already added.">>),
                                    State;
                                _ ->
                                    case installer_storage:check_storage_path_on_hosts(Workers, StoragePath) of
                                        ...
                                        {error, {hosts, Hosts}} ->
                                            onepanel_gui_utils:message(<<"top_menu">>, error, <<"Storage is not available on hosts: ",
                                            (onepanel_gui_utils:format_list(Hosts))/binary>>),
                                            State;
                                        _ ->
                                            onepanel_gui_utils:message(<<"top_menu">>, error, <<"Cannot add storage path.">>),
                                            State
                                    end
                            end,
                NextState;
