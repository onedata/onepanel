                       ...
                       {create_space, Name, Token} ->
                           NextState =
                               try
                                   ...
                                   add_space_row(RowId, SpaceDetails),
                                   onepanel_gui_utils:message(<<"top_menu">>, success, <<"Created Space's ID: <b>", SpaceId/binary, "</b>">>),
                                   ...
                                   _:Reason ->
                                       ...
                                       onepanel_gui_utils:message(<<"top_menu">>, error, <<"Cannot create Space <b>", Name/binary, "</b> associated with token <b>", Token/binary, "</b>.<br>
                       ...
                       {support_space, Token} ->
                           NextState =
                               try
                                   ...
                                   {ok, SpaceDetails} = gr_providers:get_space_details(provider, SpaceId),
                                   add_space_row(RowId, SpaceDetails),
                                   onepanel_gui_utils:message(<<"top_menu">>, success, <<"Supported Space's ID: <b>", SpaceId/binary, "</b>">>),
                                   State#?STATE{counter = Counter + 1, spaces_details = [{RowId, SpaceDetails} | SpacesDetails]}
                               catch
                                   _:Reason ->
                                       ?error("Cannot support Space associated with token ~p: ~p", [Token, Reason]),
                                       onepanel_gui_utils:message(<<"top_menu">>, error, <<"Cannot support Space associated with token <b>", Token/binary, "</b>.<br>
                            Please try again later.">>),
                                       State
                               end,
                       ...
                       {revoke_space_support, RowId, SpaceId} ->
                           NextState =
                               case gr_providers:revoke_space_support(provider, SpaceId) of
                                   ok ->
                                       onepanel_gui_utils:message(<<"top_menu">>, success, <<"Space: <b>", SpaceId/binary, "</b> is no longer supported.">>),
                                       gui_jq:remove(RowId),
                                       State#?STATE{spaces_details = proplists:delete(RowId, SpacesDetails)};
                                   Other ->
                                       ?error("Cannot revoke support for Space ~p: ~p", [SpaceId, Other]),
                                       onepanel_gui_utils:message(<<"top_menu">>, error, <<"Cannot revoke support for Space <b>", SpaceId/binary, "</b>.<br>Please try again later.">>),
                                       State
                               end,
                           NextState;
...
event(init) ->
    try
        ...
        _:Reason ->
            ...
            onepanel_gui_utils:message(<<"top_menu">>, error, <<"Cannot fetch supported Spaces.<br>Please try again later.">>)
    end;
