...
-   VFS-5838 Significant internal logic refactor. Includes minor API
    ...
    -   Endpoints creating a long-running task return a JSON body with
        ...
        202 Accepted.
    -   POST requests \'support\_space\' and \'add\_onezone\_user\' now
        use the 201 Created HTTP code, return the id of created resource
...
-   VFS-6108 Fix access token verification procedure to use onepanel\'s
    serviceToken
...
-   VFS-3986 GUI: Added Let's Encrypt certificate setup step in provider
    cluster deployment; improved error backend error descriptions
