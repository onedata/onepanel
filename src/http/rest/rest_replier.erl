...
%%
%%%%--------------------------------------------------------------------
%%%% @doc Sets a cookie with zeroed TTL
%%%% @end
%%%%--------------------------------------------------------------------
%%-spec unset_cookie(Req :: cowboy_req:req(), Name :: binary(), Path :: string()) ->
%%    cowboy_req:req().
%%unset_cookie(Req, Name, Opts) ->
%%    cowboy_req:set_resp_cookie(Name, <<>>, Req, Opts#{max_age => 0}).
