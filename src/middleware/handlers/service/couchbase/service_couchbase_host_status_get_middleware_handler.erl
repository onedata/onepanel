...
%%%-------------------------------------------------------------------
%%% @doc
%%% Returns couchbase status on a single host (OP/OZ).
%%% @end
%%%-------------------------------------------------------------------
-module(service_couchbase_host_status_get_middleware_handler).
-author("Bartosz Walkowicz").
...
process(#onp_req_state{ctx = #onp_req_ctx{gri = #gri{id = HostBin}}}) ->
    ...
    service_middleware_handler_utils:status_on_host(?SERVICE_CB, Host).
