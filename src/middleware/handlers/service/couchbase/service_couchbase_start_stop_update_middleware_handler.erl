...
%%%-------------------------------------------------------------------
%%% @doc
%%% Starts or stops couchbase on a single host (OP/OZ).
%%% TODO VFS-13023 test endpoint?
%%% @end
%%%-------------------------------------------------------------------
-module(service_couchbase_start_stop_update_middleware_handler).
-author("Bartosz Walkowicz").
...
validate(#onp_req_state{ctx = #onp_req_ctx{gri = #gri{id = <<HostBin/binary>>}}}) ->
    service_middleware_handler_utils:ensure_has_host(?SERVICE_CB, binary_to_list(HostBin)).


-spec process(state()) -> ok | errors:error().
process(#onp_req_state{ctx = #onp_req_ctx{gri = #gri{id = <<HostBin/binary>>}}, input = Data}) ->
    Host = binary_to_list(HostBin),
    service_middleware_handler_utils:set_started_on_host(?SERVICE_CB, Host, maps:get(started, Data)).
