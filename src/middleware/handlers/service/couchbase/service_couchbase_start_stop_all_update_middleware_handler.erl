...
%%%-------------------------------------------------------------------
%%% @doc
%%% Starts or stops couchbase on all hosts (OP/OZ).
%%% @end
%%%-------------------------------------------------------------------
-module(service_couchbase_start_stop_all_update_middleware_handler).
-author("Bartosz Walkowicz").
...
process(#onp_req_state{input = Data}) ->
    service_middleware_handler_utils:set_started_all(?SERVICE_CB, maps:get(started, Data)).
