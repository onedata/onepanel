...
%%%-------------------------------------------------------------------
%%% @doc
%%% Returns all hosts status for op_worker (OP only).
%%% @end
%%%-------------------------------------------------------------------
-module(service_op_worker_all_hosts_status_get_middleware_handler).
-author("Bartosz Walkowicz").
...
process(_) ->
    service_middleware_handler_utils:status_all_hosts(?SERVICE_OPW).
