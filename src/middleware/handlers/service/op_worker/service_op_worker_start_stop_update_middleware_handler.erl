...
%%%-------------------------------------------------------------------
%%% @doc
%%% Starts or stops op_worker on a single host (OP only).
%%% TODO VFS-13023 test endpoint?
%%% @end
%%%-------------------------------------------------------------------
-module(service_op_worker_start_stop_update_middleware_handler).
-author("Bartosz Walkowicz").
...
validate(#onp_req_state{ctx = #onp_req_ctx{gri = #gri{id = <<HostBin/binary>>}}}) ->
    ...
    service_middleware_handler_utils:ensure_has_host(?SERVICE_OPW, Host).


-spec process(state()) -> ok | errors:error().
process(#onp_req_state{ctx = #onp_req_ctx{gri = #gri{id = <<HostBin/binary>>}}, input = Data}) ->
    Host = binary_to_list(HostBin),
    service_middleware_handler_utils:set_started_on_host(?SERVICE_OPW, Host, maps:get(started, Data)).
