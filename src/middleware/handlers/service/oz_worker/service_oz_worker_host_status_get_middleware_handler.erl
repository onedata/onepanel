...
%%%-------------------------------------------------------------------
%%% @doc
%%% Returns oz_worker status on a single host (OZ only).
%%% @end
%%%-------------------------------------------------------------------
-module(service_oz_worker_host_status_get_middleware_handler).
-author("Bartosz Walkowicz").
...
validate(#onp_req_state{ctx = #onp_req_ctx{gri = #gri{id = <<HostBin/binary>>}}}) ->
    service_middleware_handler_utils:ensure_has_host(?SERVICE_OZW, binary_to_list(HostBin)).
...
process(#onp_req_state{ctx = #onp_req_ctx{gri = #gri{id = HostBin}}}) ->
    ...
    service_middleware_handler_utils:status_on_host(?SERVICE_OZW, Host).
