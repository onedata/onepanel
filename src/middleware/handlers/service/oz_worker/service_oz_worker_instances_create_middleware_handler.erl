...
%%%-------------------------------------------------------------------
%%% @doc
%%% Adds oz_worker nodes (OZ only).
%%% @end
%%%-------------------------------------------------------------------
-module(service_oz_worker_instances_create_middleware_handler).
-author("Bartosz Walkowicz").
...
validate(#onp_req_state{input = Data}) ->
    ...
    service_middleware_handler_utils:validate_hosts_not_existing(?SERVICE_OZW, NewHosts).


-spec process(state()) -> {ok, output()} | errors:error().
process(#onp_req_state{input = Data}) ->
    NewHosts = service_middleware_handler_utils:extract_new_hosts(Data),
    {ok, service:apply_async(?SERVICE_OZW, add_nodes, #{new_hosts => NewHosts})}.
