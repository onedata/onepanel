...
%%%-------------------------------------------------------------------
%%% @doc
%%% Middleware handler for returning Onezone user details (oz_panel only).
%%% @end
%%%-------------------------------------------------------------------
-module(user_instance_get_middleware_handler).
-author("Bartosz Walkowicz").
...
translate_output(#onp_req_state{ctx = #onp_req_ctx{interface = rest}}, UserData) ->
    {ok, ?OK_REPLY(UserData)}.
