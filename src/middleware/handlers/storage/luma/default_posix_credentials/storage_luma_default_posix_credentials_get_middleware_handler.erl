...
%%%-------------------------------------------------------------------
%%% @doc
%%% Returns default POSIX credentials (local or non-local depending on aspect).
%%% TODO VFS-13068 test endpoint
%%% @end
%%%-------------------------------------------------------------------
-module(storage_luma_default_posix_credentials_get_middleware_handler).
-author("Bartosz Walkowicz").
...
process(#onp_req_state{ctx = #onp_req_ctx{gri = #gri{aspect = {Aspect, SpaceId}, id = StorageId}}}) ->
    middleware_handler_utils:service_call(
        ?SERVICE_OPW, get_default_posix_credentials, #{
            id => StorageId,
            spaceId => SpaceId,
            isLocalFeedLumaRequest => case Aspect of
                local_feed_luma_default_posix_credentials -> true;
                luma_default_posix_credentials -> false
            end
        }
    ).
