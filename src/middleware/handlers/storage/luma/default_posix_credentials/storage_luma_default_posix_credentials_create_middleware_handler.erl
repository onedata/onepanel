...
%%%-------------------------------------------------------------------
%%% @doc
%%% Creates local-feed default POSIX credentials.
%%% TODO VFS-13068 test endpoint
%%% @end
%%%-------------------------------------------------------------------
-module(storage_luma_default_posix_credentials_create_middleware_handler).
-author("Bartosz Walkowicz").
...
process(#onp_req_state{
    ...
    middleware_handler_utils:service_exec(?SERVICE_OPW, add_default_posix_credentials, #{
        id => StorageId,
        spaceId => SpaceId,
        credentials => Data,
