...
%%%-------------------------------------------------------------------
%%% @doc
%%% Deletes mapping onedata_user -> credentials (local or non-local depending on aspect).
%%% @end
%%%-------------------------------------------------------------------
-module(storage_luma_onedata_user_to_credentials_mapping_delete_middleware_handler).
-author("Bartosz Walkowicz").
    ...
    middleware_utils:execute_service_action(
        ?SERVICE_OPW, remove_onedata_user_to_credentials_mapping, #{
            ...
            isLocalFeedLumaRequest => case Aspect of
                local_feed_luma_onedata_user_to_credentials_mapping -> true;
