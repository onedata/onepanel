...
%%%-------------------------------------------------------------------
%%% @doc
%%% Creates local-feed mapping acl_group -> onedata_group.
%%% TODO VFS-13023 test endpoint
%%% @end
%%%-------------------------------------------------------------------
-module(storage_luma_acl_group_to_onedata_group_mapping_create_middleware_handler).
-author("Bartosz Walkowicz").
...
process(#onp_req_state{
    ...
    middleware_handler_utils:service_exec(?SERVICE_OPW, add_acl_group_to_onedata_group_mapping, #{
        id => StorageId,
        aclGroup => AclGroup,
        onedataGroup => Data,
