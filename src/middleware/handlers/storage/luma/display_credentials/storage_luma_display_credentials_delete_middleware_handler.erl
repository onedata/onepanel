...
%%%-------------------------------------------------------------------
%%% @doc
%%% Deletes display credentials (local or non-local depending on aspect).
%%% @end
%%%-------------------------------------------------------------------
-module(storage_luma_display_credentials_delete_middleware_handler).
-author("Bartosz Walkowicz").
    ...
    middleware_utils:execute_service_action(
        ?SERVICE_OPW, remove_display_credentials, #{
            ...
            isLocalFeedLumaRequest => case Aspect of
                local_feed_luma_display_credentials -> true;
                luma_display_credentials -> false
            end
        }
    ).
