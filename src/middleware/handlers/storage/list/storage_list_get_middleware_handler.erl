...
%%%-------------------------------------------------------------------
%%% @doc
%%% Returns list of storage ids (OP only).
%%% @end
%%%-------------------------------------------------------------------
-module(storage_list_get_middleware_handler).
-author("Bartosz Walkowicz").
...
-type output() :: [op_worker_storage:id()].
...
translate_output(#onp_req_state{ctx = #onp_req_ctx{interface = rest}}, StorageIds) ->
    {ok, ?OK_REPLY(#{<<"ids">> => StorageIds})}.
