...
%%%-------------------------------------------------------------------
%%% @doc
%%% Gets auto-cleaning configuration for a space.
%%% @end
%%%-------------------------------------------------------------------
-module(space_auto_cleaning_configuration_get_middleware_handler).
-author("Bartosz Walkowicz").
...
process(#onp_req_state{ctx = #onp_req_ctx{gri = #gri{id = SpaceId}}}) ->
    middleware_handler_utils:service_call(
        ?SERVICE_OP, get_auto_cleaning_configuration, #{space_id => SpaceId}
    ).
