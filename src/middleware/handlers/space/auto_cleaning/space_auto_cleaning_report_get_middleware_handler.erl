...
%%%-------------------------------------------------------------------
%%% @doc
%%% Gets a specified auto-cleaning report for a space.
%%% TODO VFS-13070 test endpoint
%%% @end
%%%-------------------------------------------------------------------
-module(space_auto_cleaning_report_get_middleware_handler).
-author("Bartosz Walkowicz").
...
process(#onp_req_state{ctx = #onp_req_ctx{gri = #gri{id = SpaceId, aspect = {auto_cleaning_report, ReportId}}}}) ->
    Ctx = #{space_id => SpaceId, report_id => ReportId},
    middleware_handler_utils:service_call(?SERVICE_OP, get_auto_cleaning_report, Ctx).


-spec translate_output(state(), output()) -> {ok, middleware_handler:rest_output()}.
translate_output(#onp_req_state{ctx = #onp_req_ctx{interface = rest}}, Report) ->
    {ok, ?OK_REPLY(Report)}.
