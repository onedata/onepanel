...
%%%-------------------------------------------------------------------
%%% @doc
%%% Middleware handler for returning Onezone policies.
%%% @end
%%%-------------------------------------------------------------------
-module(zone_policies_get_middleware_handler).
-author("Bartosz Walkowicz").
...
process(_) ->
    middleware_handler_utils:service_call(?SERVICE_OZW, get_policies).


-spec translate_output(state(), output()) ->
    {ok, middleware_handler:rest_output()}.
translate_output(#onp_req_state{ctx = #onp_req_ctx{interface = rest}}, Policies) ->
    {ok, ?OK_REPLY(kv_utils:copy_found([
