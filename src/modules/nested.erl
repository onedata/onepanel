...
% used in functions transfering values between containers
...
%%--------------------------------------------------------------------
%% @doc Performs {@link get/2} and converts obtained result to given type.
%% @end
%%--------------------------------------------------------------------
-spec get_converted(path(K), nested(K, _), onepanel_utils:type()) -> term().
get_converted(Path, Nested, Type) ->
    onepanel_utils:convert(get(Path, Nested), Type).


%%--------------------------------------------------------------------
%% @doc Performs {@link get/3} and converts obtained result to given type.
%% @end
%%--------------------------------------------------------------------
-spec get_converted(path(K), nested(K, _), onepanel_utils:type(), Default) ->
    term() | Default.
...
%%--------------------------------------------------------------------
%% @doc Performs {@link find/3} and converts obtained result to given type.
%% @end
%%--------------------------------------------------------------------
-spec find_converted(path(K), nested(K, _), onepanel_utils:type()) ->
    {ok, term()} | error.
