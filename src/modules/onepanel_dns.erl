...
-type summary() :: ok | unresolvable | bad_records | missing_records | error.
...
% @fixme extract redundancy between check_any and check_all
...
build_bind_record(Domain, a, IP) when is_binary(IP) ->
    onepanel_utils:join([Domain, <<"IN">>, <<"A">>, IP], <<" ">>);
...
%% @private
%% @doc
%% Converts dns_data value retuned by inet_res to expected format.
%% @end
%%--------------------------------------------------------------------
...
compare(#dns_check{summary = SameSummary, excessive = Add1},
    #dns_check{summary = SameSummary, excessive = Add2}) ->
    length(Add1) >= length(Add2);
...
-spec log_result(Names :: [dns_name()], Type :: dns_type(),
    Servers :: [inet:ip4_address()], Result :: dns_check()) -> dns_check().
