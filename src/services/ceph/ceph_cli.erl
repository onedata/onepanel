...
-define(QUOTE(Tokens),
    [shell_utils:quote(Token) || Token <- Tokens]).
...
status() ->
    json_utils:decode(shell_utils:get_success_output(?CEPH(["status"]))).
...
-spec df() -> map().
df() ->
    Output = shell_utils:get_success_output(?CEPH(["df"])),
...
stop_with_timeout(StartCommand) ->
    ...
    try
        ...
        throw:attempts_limit_exceeded ->
            ...
            shell_utils:pkill(StartCommand, 'KILL')
    end.
...
auth_add(KeyringPath, User, Caps) ->
    ...
    shell_utils:ensure_success(Cmd).
...
auth_get(Entity) ->
    shell_utils:get_success_output(?AUTH(["get", Entity])).
...
auth_get(Entity, OutputFile) ->
    shell_utils:ensure_success(?AUTH(["get", Entity, "-o", OutputFile])).


-spec auth_print_key(Name :: token()) -> Key :: binary().
auth_print_key(Name) ->
    Output = shell_utils:get_success_output(?CEPH(["auth", "print-key", Name])),
...
auth_create_keyring(Path, Name, Capabilities) ->
    ...
    shell_utils:ensure_success(?AUTHTOOL(Cmd)).
...
osd_create() ->
    JSON = shell_utils:get_success_output(?CEPH(["osd", "create"])),
...
-spec osd_create(UUID :: ceph:uuid(), Id :: service_ceph_osd:id()) ->
    ok.
osd_create(UUID, Id) ->
    JSON = shell_utils:get_success_output(?CEPH(["osd", "create", UUID, <<"osd.", Id/binary>>])),
...
osd_next_id() ->
    JSON = shell_utils:get_success_output(?CEPH(["osd", "getmaxosd"])),
...
osd_mkfs_mkkey(Id) ->
    shell_utils:ensure_success(?OSD(["-i", Id, "--mkfs", "--mkkey", "--no-mon-config"])).


-spec osd_start(Id :: service_ceph_osd:id()) -> ok | no_return().
osd_start(Id) ->
    shell_utils:ensure_success(osd_start_cmd(Id)).
...
osd_mark(Id, State) ->
    shell_utils:ensure_success(?CEPH(["osd", State, Id])).


-spec osd_purge(Id :: service_ceph_osd:id()) -> ok | no_return().
osd_purge(Id) ->
    shell_utils:ensure_success(?CEPH([
        "osd", "purge", Id, "--yes-i-really-mean-it"])).


-spec osd_is_safe_to_destroy(Id :: service_ceph_osd:id()) -> boolean().
osd_is_safe_to_destroy(Id) ->
    {Code, _Output, _} = shell_utils:execute(?CEPH(["osd", "safe-to-destroy", Id])),
    Code == 0.


-spec osd_df() -> #{binary() := term()}.
osd_df() ->
    Output = shell_utils:get_success_output(?CEPH(["osd", "df", "plain"])),
    json_utils:decode(Output).
...
list_pools() ->
    JSON = shell_utils:get_success_output(?CEPH(["osd", "lspools"])),
...
get_pool_param(Pool, Param) ->
    Output = shell_utils:get_success_output(?CEPH(["osd", "pool", "get", Pool, Param])),
...
volume_list() ->
    Output = shell_utils:get_success_output(?VOLUME_LVM(["list", "--format=json"])),
    json_utils:decode(Output).
...
volume_activate(OsdId, OsdUUID) ->
    ...
    shell_utils:ensure_success(?VOLUME_LVM(
...
mon_start(Id, DataDir, IP) ->
    shell_utils:ensure_success(mon_start_cmd(Id, DataDir, IP)).
...
mon_export_monmap(OutputPath) ->
    {done, _} = ?TIMEOUT(
        shell_utils:ensure_success(?CEPH(["mon", "getmap", "-o", OutputPath]))
    ),
    ok.
...
mon_extract_monmap(MonId, OutputPath) ->
    shell_utils:ensure_success(?MON(["-i", MonId, "--extract-monmap", OutputPath])).


-spec mgr_start(Name :: token()) -> ok.
mgr_start(Name) ->
    shell_utils:ensure_success(mgr_start_cmd(Name)).
