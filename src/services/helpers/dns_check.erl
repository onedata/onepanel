...
%%% on demand when triggered by REST request. Result of last check
...
%% @end
%%--------------------------------------------------------------------
-spec get(worker_service(), Refresh :: true | false | never) -> result() | no_return().
...
check(_Service = oz_worker, dnsZone) ->
    Domain = binary_to_list(service_oz_worker:get_domain()),
    DnsServers = get_dns_servers(),
...
%% @end
%%--------------------------------------------------------------------
-spec lookup_ns_names(Domain :: string(), DnsServer :: inet:ip4_address()) -> [string()].
