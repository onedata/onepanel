...
-spec build_credentials(map()) -> onedata_storage:posix_helper_credentials().
build_credentials(Params) ->
    #posix_helper_credentials{
        uid = maps:get(rootUid, Params, 0),
        gid = maps:get(rootGid, Params, 0)
    }.


-spec build_credentials_diff(map()) -> onedata_storage:posix_helper_credentials_diff().
build_credentials_diff(Params) ->
    #posix_helper_credentials_diff{
        uid = maps:get(rootUid, Params, undefined),
        gid = maps:get(rootGid, Params, undefined)
    }.


-spec build_configuration(map()) -> onedata_storage:posix_helper_configuration().
build_configuration(Params) ->
    #posix_helper_configuration{
...
-spec build_configuration_diff(map()) -> onedata_storage:posix_helper_configuration_diff().
build_configuration_diff(Params) ->
    #posix_helper_configuration_diff{
        mount_point = maps:get(mountPoint, Params, undefined)
    }.


-spec credentials_to_map(onedata_storage:posix_helper_credentials()) -> map().
credentials_to_map(#posix_helper_credentials{uid = Uid, gid = Gid}) ->
    Base = #{rootUid => Uid},
    maps_utils:put_if_defined(Base, rootGid, Gid).


-spec configuration_to_map(onedata_storage:posix_helper_configuration()) -> map().
configuration_to_map(#posix_helper_configuration{
